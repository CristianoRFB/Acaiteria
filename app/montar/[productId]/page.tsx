...
export default function ConfiguratorPage() {
  ...
  return <main className="min-h-screen min-w-0 bg-[#fffaf5] pb-32 text-[#2b1722] lg:pb-12">
    <header className="sticky top-0 z-30 border-b border-[#82204f]/10 bg-[#fffaf5]/94 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-3 px-4 sm:h-18 sm:px-6">
        ...
        <div className="min-w-0 flex-1 text-center"><strong className="block truncate text-sm">Monte seu pedido</strong><span className="block truncate text-xs text-[#826a75]">{product.name}</span></div>
    ...
    <div className="mx-auto grid max-w-6xl gap-6 px-4 py-5 sm:gap-8 sm:px-6 sm:py-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:py-10">
      <div className="min-w-0">
        <div className="overflow-hidden rounded-[28px] bg-[#53142f] text-white">
          <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_88px] items-center gap-3 p-5 min-[420px]:grid-cols-[minmax(0,1fr)_120px] sm:grid-cols-[minmax(0,1fr)_180px] sm:p-6">
            <div className="min-w-0"><span className="text-[11px] font-bold uppercase tracking-[.16em] text-[#ffcf3d] sm:text-xs">Do seu jeito</span><h1 className="mt-2 break-words text-2xl font-black tracking-[-.04em] sm:text-3xl">{product.name}</h1><p className="mt-2 text-sm leading-relaxed text-white/70">{product.description}</p>{development && <span className="mt-3 inline-block rounded-full bg-white/10 px-2.5 py-1 text-[10px] font-bold">DESENVOLVIMENTO</span>}</div>
            {product.imageUrl && <img className="aspect-square min-w-0 w-full rounded-[18px] object-cover sm:rounded-[22px]" src={product.imageUrl} alt="Imagem do produto" />}
          </div>
        </div>
    ...
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-[#82204f]/10 bg-white/95 px-3 pb-[calc(.75rem+env(safe-area-inset-bottom))] pt-3 shadow-[0_-12px_35px_rgba(53,25,36,.1)] backdrop-blur-xl lg:hidden"><div className="mx-auto flex max-w-lg items-center gap-2 min-[380px]:gap-3"><div className="min-w-0 flex-1"><strong className="block truncate text-sm">{size?.label ?? 'Escolha um tamanho'} • {itemCount} itens</strong><span className="text-lg font-black text-[#82204f]">{formatBRL(preview)}</span></div><Button onClick={submit} className="h-12 shrink-0 rounded-full bg-[#82204f] px-4 text-sm font-black text-white min-[380px]:px-5">{editId ? 'Salvar' : 'Adicionar'} <ChevronRight className="size-4" /></Button></div>{error && <p role="alert" className="mx-auto mt-2 max-w-lg text-xs font-bold text-red-700">{error}</p>}</div>
  </main>;
}
