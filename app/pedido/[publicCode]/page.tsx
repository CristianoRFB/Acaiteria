...
interface PublicOrder {
  ...
  items: PricedItem[]; pricing: { totalCents: number }; fulfillment: { mode: 'PICKUP' | 'DELIVERY' }; status: OrderStatus; editProposal?: PublicOrderEditProposal;
}
...
export default function OrderPage() {
  ...
  const displayOrder = order.editProposal?.status === 'REJECTED' ? { ...order, items: order.editProposal.before.items, pricing: { ...order.pricing, totalCents: order.editProposal.before.pricing.totalCents }, fulfillment: { ...order.fulfillment, mode: order.editProposal.before.fulfillment.mode } } : order;
  const steps = order.fulfillment.mode === 'PICKUP' ? baseSteps.filter((step) => !step.statuses.includes('OUT_FOR_DELIVERY')) : baseSteps;
  const activeIndex = steps.findIndex((step) => step.statuses.includes(order.status));
  const estimate = getOrderEstimateForStatus(order.status, order.estimatedMinutes ?? config.orderEstimateMinutes ?? 15);
  const orderCode = (order.orderNumber || publicCode).replace(/^#/, '');
  const statusLabel = { NEW: 'Solicitação recebida', CONFIRMED: 'Pedido confirmado', PREPARING: 'Pedido em preparo', READY: 'Pronto para retirada', OUT_FOR_DELIVERY: 'Saiu para entrega', COMPLETED: 'Pedido concluído', CANCELLED: 'Pedido cancelado' }[order.status];
  const copyCode = async () => { try { await copyText(orderCode); setCopied(true); window.setTimeout(() => setCopied(false), 2200); } catch { setCopied(false); } };
  const updated = order.updatedAt ? formatUpdatedAt(order.updatedAt) : null;
  return <main className="min-h-screen min-w-0 bg-[#fffaf5] text-[#2b1722]"><PublicHeader /><div className="mx-auto min-w-0 max-w-3xl px-4 py-6 sm:px-6 sm:py-10">
    {order.editProposal && <section className={`mb-5 rounded-[28px] p-6 shadow-sm ${order.editProposal.status === 'PENDING' ? 'border-2 border-[#d7f04a] bg-[#fffde8]' : 'bg-white'}`}><p className="text-xs font-black uppercase tracking-wider text-[#a62c63]">Alteração no pedido</p><h2 className="mt-1 text-xl font-black">{order.editProposal.status === 'PENDING' ? 'A loja atualizou seu pedido' : order.editProposal.status === 'ACCEPTED' ? 'Alterações aceitas' : 'Alterações recusadas'}</h2><p className="mt-2 text-sm leading-relaxed text-[#6f5360]">{order.editProposal.status === 'PENDING' ? order.editProposal.summary : order.editProposal.status === 'ACCEPTED' ? 'Obrigado! A loja recebeu sua confirmação e seguirá com o pedido.' : 'Você recusou as alterações. A loja foi avisada e o pedido será restaurado à versão anterior.'}</p>{editError && <p role="alert" className="mt-3 text-sm font-bold text-red-700">{editError}</p>}{order.editProposal.status === 'PENDING' && <div className="mt-4 flex flex-col gap-2 sm:flex-row"><Button disabled={editBusy} onClick={() => void respondToEdit('ACCEPTED')} className="rounded-full bg-[#82204f] text-white">{editBusy ? 'Salvando…' : 'Concordo com as alterações'}</Button><Button disabled={editBusy} variant="outline" onClick={() => void respondToEdit('REJECTED')} className="rounded-full">Recusar alterações</Button></div>}</section>}
    {order.pricingReviewStatus === 'PENDING' && <section className="mb-5 rounded-[24px] border border-[#f0bd43] bg-[#fff9df] p-4 text-sm leading-relaxed text-[#6f5360]"><strong className="text-[#351924]">Valor em conferência</strong><br />A loja está conferindo itens e total antes de aceitar o pedido. Não é preciso reenviar nada.</section>}
    <section className="overflow-hidden rounded-[32px] border-2 border-[#d2ae47] bg-[#351924] p-5 text-white shadow-[0_18px_50px_rgba(53,25,36,.22)] sm:p-8">
      <div className="flex items-center gap-4"><span className="grid size-12 shrink-0 place-items-center rounded-full bg-[#d7f04a] text-[#351924] sm:size-14"><KeyRound className="size-6 sm:size-7" /></span><span className="inline-flex min-h-10 items-center gap-2 rounded-full bg-[#64414a] px-4 py-2 text-sm font-black leading-tight text-[#d7f04a]"><Check className="size-4 shrink-0" /> {search.get('novo') === '1' ? 'Solicitação recebida' : statusLabel}</span></div>
      <h1 className="mt-7 max-w-sm text-4xl font-black leading-[1.05] tracking-[-.04em] sm:text-5xl">Guarde este código</h1>
    ...
    <div className="mt-5 flex flex-col gap-3 sm:flex-row"><Button variant="outline" className="h-12 flex-1 rounded-full" onClick={load}><RefreshCw /> Atualizar status</Button>{config.whatsappEnabled && config.whatsappNumber && <Button className="h-12 flex-1 rounded-full bg-[#1f9d55] text-white hover:bg-[#178447]" render={<a href={whatsappUrl()} target="_blank" rel="noreferrer" />}><MessageCircle /> Abrir WhatsApp</Button>}</div><p className="mt-5 text-center text-xs text-[#826a75]">{updated ? `Última atualização em ${updated}. ` : ''}O código não permite listar outros pedidos.</p>
  </div></main>;
}
