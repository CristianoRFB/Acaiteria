...
import { formatBRL, getCustomerOrderStatusMessage, type OrderStatus, type PricedItem } from '@/shared/domain';

interface PublicOrder { integrationMessage?: string; statusMessage?: string; orderNumber: string; createdAt?: string; items: PricedItem[]; pricing: { totalCents: number }; fulfillment: { mode: 'PICKUP' | 'DELIVERY' }; status: OrderStatus; updatedAt?: string }
...
export default function OrderPage() {
  ...
  return <main className="min-h-screen bg-[#fffaf5] text-[#2b1722]"><PublicHeader /><div className="mx-auto max-w-3xl px-4 py-10 sm:px-6"><div className="rounded-[32px] bg-[#351924] p-6 text-white sm:p-9">{search.get('novo') === '1' && <span className="inline-flex items-center gap-2 rounded-full bg-[#d7f04a] px-3 py-1.5 text-xs font-black text-[#351924]"><Check className="size-3.5" /> Pedido salvo</span>}<p className="mt-5 text-sm text-white/60">Acompanhe seu pedido</p><h1 className="mt-1 text-4xl font-black tracking-[-.05em]">{order.orderNumber}</h1><p role="status" aria-live="polite" className="mt-3 rounded-2xl bg-white/10 p-4 text-sm font-bold leading-relaxed text-white">{order.statusMessage ?? getCustomerOrderStatusMessage(order.status) ?? order.integrationMessage ?? 'Pedido recebido pela loja. Acompanhe a atualização nesta página.'}</p><div className="mt-8 grid grid-cols-5 gap-1">{steps.map((step, index) => { const Icon = step.icon; const done = activeIndex >= index && order.status !== 'CANCELLED'; return <div key={step.label} className="text-center"><span className={`mx-auto grid size-9 place-items-center rounded-full ${done ? 'bg-[#d7f04a] text-[#351924]' : 'bg-white/10 text-white/35'}`}><Icon className="size-4" /></span><span className={`mt-2 block text-[10px] font-bold ${done ? 'text-white' : 'text-white/35'}`}>{step.label}</span></div>; })}</div>{order.status === 'CANCELLED' && <div className="mt-6 rounded-2xl bg-red-400/15 p-4 text-sm font-bold text-red-100">Este pedido foi cancelado. Entre em contato com a loja se precisar de ajuda.</div>}</div>
