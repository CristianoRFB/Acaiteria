  ...
  return (
    <AdminShell adminOnly>
      ...
      <section className="mt-7 rounded-[26px] bg-white p-5 shadow-sm sm:p-7">
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="text-xs font-black uppercase tracking-wider text-[#a62c63]">Movimentações</p><h2 className="mt-1 text-2xl font-black">Caixa da açaíteria</h2><p className="mt-1 text-sm text-[#826a75]">Use os filtros para conferir um mês ou separar receitas e despesas.</p></div><div className="grid min-w-0 grid-cols-2 gap-3 sm:flex sm:flex-row"><label className="block min-w-0 text-xs font-bold text-[#826a75]">Mês<input type="month" aria-label="Filtrar por mês" value={month} onChange={(event) => setMonth(event.target.value)} className="mt-1 h-10 w-full min-w-0 rounded-xl border border-[#82204f]/15 bg-[#fffaf5] px-3 text-sm font-bold" /></label><label className="block min-w-0 text-xs font-bold text-[#826a75]">Exibir<select aria-label="Filtrar tipo de lançamento" value={kind} onChange={(event) => setKind(event.target.value as 'ALL' | FinanceEntryKind)} className="mt-1 h-10 w-full min-w-0 rounded-xl border border-[#82204f]/15 bg-[#fffaf5] px-3 text-sm font-bold"><option value="ALL">Tudo</option><option value="INCOME">Receitas</option><option value="EXPENSE">Despesas</option></select></label></div></div>
