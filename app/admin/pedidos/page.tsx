...
export default function OrdersPage() {
  ...
  return <AdminShell><div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="eyebrow">Operação</p><h1 className="section-title">Pedidos</h1><p className="mt-2 text-sm text-text-muted">Veja a fila por estágio e saiba qual é a próxima ação.</p></div><a href="/admin/ajuda" className="inline-flex min-h-10 w-fit items-center gap-2 rounded-full border border-border-soft bg-white px-4 text-sm font-black text-brand hover:bg-surface-warm">Como operar <ArrowRight className="size-4" /></a></div>
    <div className="action-bar mt-6"><label className="relative block min-w-0 flex-1"><Search className="pointer-events-none absolute left-4 top-1/2 size-4 -translate-y-1/2 text-brand" /><span className="sr-only">Procurar pedido</span><input aria-label="Procurar pedido" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Número, código, cliente ou WhatsApp" className="h-12 w-full rounded-full border border-border-soft bg-surface-warm pl-11 pr-4 text-sm font-bold outline-none focus:border-brand focus:ring-2 focus:ring-brand/15" /></label><div className="flex max-w-full gap-2 overflow-x-auto pb-1" role="tablist" aria-label="Filtrar pedidos">{filters.map((status) => <button key={status} type="button" role="tab" aria-selected={filter === status} onClick={() => setFilter(status)} className={`min-h-10 shrink-0 rounded-full px-4 text-xs font-black transition ${filter === status ? 'bg-brand-deep text-white' : 'bg-surface-warm text-text-muted hover:text-brand'}`}>{status === 'ACTIVE' ? 'Em andamento' : statusLabel[status]}</button>)}</div></div>
