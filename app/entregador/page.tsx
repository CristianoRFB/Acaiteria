...
interface DriverProfile { name?: string; phone?: string; status?: DeliveryDriverStatus; enabled?: boolean }
...
export default function DriverHomePage() {
  ...
  useEffect(() => {
    ...
    return () => { stopProfile(); stopDeliveries(); };
  }, [role, user]);
  useEffect(() => { if (!loading && (!user || role !== 'driver')) window.location.href = '/entregador/login'; }, [loading, role, user]);
  const current = useMemo(() => deliveries.find((item) => ['ASSIGNED', 'ACCEPTED', 'ON_THE_WAY', 'ARRIVED'].includes(item.status)), [deliveries]);
  const next = useMemo(() => deliveries.filter((item) => item.status === 'ASSIGNED' && item.id !== current?.id), [current?.id, deliveries]);
  const history = useMemo(() => deliveries.filter((item) => ['DELIVERED', 'DELIVERY_FAILED', 'CANCELLED'].includes(item.status)).sort((a, b) => String(b.updatedAt ?? '').localeCompare(String(a.updatedAt ?? ''))), [deliveries]);
