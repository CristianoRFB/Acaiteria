...
export function AdminNotifications() {
  ...
  async function quickAction(orderId: string, status: Extract<OrderStatus, 'CONFIRMED' | 'CANCELLED'>) {
    ...
    setMessage('');
    try {
      await httpsCallable(getFirebaseClient().functions, 'updateOrderStatus')({ orderId, status, ...(status === 'CANCELLED' ? { reason: 'Recusado pela loja' } : {}) });
    ...
    <div className="fixed bottom-5 right-5 z-40 flex flex-col items-end gap-3">
      {open && (
        <section className="w-[min(92vw,380px)] overflow-hidden rounded-3xl border border-[#82204f]/10 bg-white shadow-2xl" aria-label="Pedidos novos">
          ...
          {message && <p role="status" className="border-b bg-amber-50 px-5 py-3 text-xs font-bold text-amber-900">{message}</p>}
