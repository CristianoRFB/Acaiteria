...
const valid = { clientRequestId: '8bc68d79-8242-4def-8eb1-3d22a6f56d37', customer: { name: 'Cliente Teste', whatsapp: '5517999999999' }, items: [{ productId: 'p', sizeId: 's', quantity: 1, selections: [] }], fulfillment: { mode: 'PICKUP' }, payment: { method: 'PIX' }, clientPreviewTotalCents: 1 };
describe('validação da Function createOrder', () => {
  ...
  it('rejeita troco quando pagamento não é dinheiro', () => expect(createOrderSchema.safeParse({ ...valid, payment: { method: 'PIX', changeForCents: 5000 } }).success).toBe(false));
